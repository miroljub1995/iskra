...
public static class Program
{
    static async Task Main(string[] args)
    {
        ...
        w.Console.Log("this is from console", w.JSObject, w.Document.JSObject);

        var eventListener = new EventListener(SomeListener);

        w.Document.AddEventListener("test", eventListener, false);
        w.Document.DispatchEvent(Event.New("test"));
        w.Document.DispatchEvent(Event.New("test"));
        w.Document.DispatchEvent(Event.New("test"));

        Console.WriteLine("Removing event listener...");
        w.Document.RemoveEventListener("test", eventListener, false);
        w.Document.DispatchEvent(Event.New("test"));
        w.Document.DispatchEvent(Event.New("test"));
        w.Document.DispatchEvent(Event.New("test"));
        ...
        w.Console.Log("Appended child", newDiv.JSObject);
        ...
        {
            w.Console.Log("before full screen", e.JSObject);
    ...
    {
        var w = JSObjectProxyFactory.GetProxy<Window>(JSHost.GlobalThis);
        w.Console.Log("this is from listener", e.JSObject);
    }
}