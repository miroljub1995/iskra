...
public class HydrationStateFeatureTests
{
    ...
    public async Task Renders_empty_script_when_value_is_empty()
    {
        ...
        await Assert.That(output).IsEqualTo(
            $"<script id=\"{HydrationStateScript.DefaultScriptElementId}\" type=\"application/json\">{{\"hydrate\":true}}</script>");
    }
    ...
    public async Task Renders_serialized_json_when_value_is_populated()
    {
        ...
        await Assert.That(output).IsEqualTo(
            $"<script id=\"{HydrationStateScript.DefaultScriptElementId}\" type=\"application/json\">{{\"hydrate\":true,\"count\":42,\"name\":\"iskra\"}}</script>");
    }
    ...
    public async Task State_survives_unmount_and_can_be_re_read()
    {
        ...
        await Assert.That(feature.Dehydrate().Count).IsEqualTo(2);
    ...
    public async Task Renders_script_with_custom_element_id()
    {
        ...
        await Assert.That(output).IsEqualTo(
            $"<script id=\"{customId}\" type=\"application/json\">{{\"hydrate\":true}}</script>");
    }
}
